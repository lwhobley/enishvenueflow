...
import { tipPools, tipPoolEntries, payrollRecords, timeClockEntries, users } from "@workspace/db";
...
router.post("/tip-pools/:id/distribute", async (req, res) => {
  try {
    ...
    // Get all time clock entries for the venue around the schedule period
    const venueUsers = await db.select().from(users).where(eq(users.venueId, pool.venueId));
    const total = parseFloat(pool.totalTips);
    const count = venueUsers.length;

    // Delete old entries
    await db.delete(tipPoolEntries).where(eq(tipPoolEntries.poolId, id));

    // Distribute based on method
    const insertData = venueUsers.map((u, i) => {
      let tipAmount = 0;
      if (pool.distributionMethod === "equal") {
        tipAmount = count > 0 ? total / count : 0;
      } else {
        tipAmount = count > 0 ? total / count : 0;
      }
      return {
